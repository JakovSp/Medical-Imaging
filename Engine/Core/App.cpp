...
App::App(const shared_ptr<IRenderer>& renderer3D) :
...
void App::Initialize(CoreApplicationView^ applicationView)
{
	...
	_vanitycore = std::make_shared<VanityCore>();
}
...
void App::Load(Platform::String^ entryPoint)
{
	...
	if (_main == nullptr)
		_main = std::unique_ptr<MainLoop>(new MainLoop(_vanitycore, _renderer3D));
}
...
void App::Run()
{
	...
	while (!m_windowClosed) {
		...
		if (m_windowVisible) {
			...
			if (_main->Render()) _vanitycore->Present();
		}
		else
...
void App::OnWindowSizeChanged(CoreWindow^ sender, WindowSizeChangedEventArgs^ args)
{
	...
	_vanitycore->SetLogicalSize(Size(sender->Bounds.Width, sender->Bounds.Height));
	_main->CreateWindowSizeDependentResources();
}
...
void App::OnDpiChanged(DisplayInformation^ sender, Object^ args)
{
	...
	_vanitycore->SetDpi(sender->LogicalDpi);
	_main->CreateWindowSizeDependentResources();
}
...
void App::OnOrientationChanged(DisplayInformation^ sender, Object^ args)
{
	...
	_vanitycore->SetCurrentOrientation(sender->CurrentOrientation);
	_main->CreateWindowSizeDependentResources();
}
